...
namespace facebook {
namespace react {
RSkBaseEventEmitter::RSkBaseEventEmitter(Instance *bridgeInstance)
...
void RSkBaseEventEmitter::addListener(std::string eventName) {
    listenerCount_++;
    if (listenerCount_ == 1) {
        RNS_LOG_ERROR("I am in AddLister");
...
void RSkBaseEventEmitter::sendEventWithName(std::string eventName, folly::dynamic &&params, EmitterCompleteVoidCallback completeCallback) {
    ...
    RNS_LOG_ERROR("I am suppose to send the Event");
    // TODO: check if the eventName is in supportedEvents()
    if (listenerCount_ >= 1) {
        RNS_LOG_ERROR("I am sending the Event from native"<<params);
